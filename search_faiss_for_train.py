...
if __name__ == '__main__':
    ...
    query_embeddings = np.load(os.path.join(args.model_dir, "train_embeddings.npy")).astype(np.float32)
    query_labels = np.load(os.path.join(args.model_dir, "train_labels.npy"))
    ...
    false_limit = 100
    gpu_index.add(query_embeddings)  # add vectors to the index
    ...
    for i, q_label in enumerate(query_labels):
        searched_indices = query_labels[search_idx[i]][1:]
    ...
    json.dump(true_indices, open(os.path.join(args.model_dir, "train_true_indices.json"), "w+"))
    json.dump(false_indices, open(os.path.join(args.model_dir, "train_false_indices.json"), "w+"))
    json.dump(accuracy_list, open(os.path.join(args.model_dir, "train_accuracy_list.json"), "w+"))
    json.dump(accuracies, open(os.path.join(args.model_dir, "train_accuracies.json"), "w+"))
    print(accuracy_list)
